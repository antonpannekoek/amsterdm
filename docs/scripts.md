...
- The `candidate` module wraps around an input data file. This does read the full file directly into memory, since most operations require the full data (for example, the background estimation)[^1]

  The wrapped data is contained in a `Candidate` class instance; the methods are the same as (or very similar to) the core functions, just with the first argument skipped (as this is the instance).

  A `Candidate` instance contains a `header` and a `data` attribute, which contain what their name suggest: header information in the form of a dict (or a FITS header), and the three-dimensional data.

  There are a few special properties: `freqs` and `times`, which are conversions from the channels and samples. Be aware that these are cached properties: if anything changes under the hood (e.g., you change the channel offset or the start time), the properties need to be recalculated: simply delete the property (e.g., `del candidate.times`) and use it again: it will be recalculated automatically that way. Note that `times` are the MJD, deduced from the actual observation start time (`tstart` in the header).

- The `plot` module relies on a `Candidate` instance as the first argument, and contains functions to create a variety of plots, using Matplotlib. The routines can also take an optional `ax` argument, on which the plot(s) will be drawn if given. If not, a new figure is created under the hood by the routines.
